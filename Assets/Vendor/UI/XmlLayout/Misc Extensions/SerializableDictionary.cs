...
namespace UI.Xml
{
    ...
    public class SerializableDictionary<TKey, TValue> : IDictionary<TKey, TValue>
    {
        ...
        private int FindIndex(TKey key)
        {
            ...
            {
                if (_Comparer == null) { _Comparer = EqualityComparer<TKey>.Default; }
                int hash = _Comparer.GetHashCode(key) & 2147483647;
        ...
        public struct Enumerator : IEnumerator<KeyValuePair<TKey, TValue>>
        {
            ...
            public bool MoveNext()
            {
                if (_Version != _Dictionary._Version)
                    throw new InvalidOperationException(string.Format("Enumerator version {0} != Dictionary version {1} (Dictionary type: {2})\r\n{3}", _Version, _Dictionary._Version, _Dictionary.GetType().Name, _Dictionary));
